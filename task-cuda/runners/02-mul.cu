...
static float runOnce(int n, int blockSize) {
  ...
  checkCuda(cudaMalloc(&dx, bytes));
  checkCuda(cudaMalloc(&dy, bytes));
  checkCuda(cudaMalloc(&dr, bytes));
  checkCuda(cudaMemcpy(dx, hx.data(), bytes, cudaMemcpyHostToDevice));
  checkCuda(cudaMemcpy(dy, hy.data(), bytes, cudaMemcpyHostToDevice));
  int grid = (n + blockSize - 1) / blockSize;
  KernelMul<<<grid, blockSize>>>(n, dx, dy, dr);
...
static void doCheck() {
  ...
  int grid = (n + blockSize - 1) / blockSize;
  KernelMul<<<grid, blockSize>>>(n, dx, dy, dr);
  checkCuda(cudaMemcpy(hr.data(), dr, bytes, cudaMemcpyDeviceToHost));
  for (int i = 0; i < n; ++i) {
    if (hr[i] != ref[i]) {
      fail("mul mismatch");
    }
